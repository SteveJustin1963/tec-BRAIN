...
# Experiments: PC vs Backprop on Toy Classification (`pc_torch.py`)

Hands-on comparison of a Predictive Coding network (local Hebbian updates only) against a standard backprop MLP (Adam optimizer) on synthetic 2D datasets.

## What was built

**File:** `pc_torch.py`

Both models share the same architecture depth but PC weights are updated purely through local Hebbian rules — no gradient is ever backpropagated through layer activities.

### Phase 1 — Baseline (3-class blobs, noise=0.5)

Initial experiment on a simple, well-separated dataset.

- Dataset: 3 Gaussian blobs evenly spaced on a circle (radius 3), 150 samples/class, `noise=0.5`
- Network shape: `2 → 32 → 16 → 3`
- PC hyperparams: `α=0.05`, `β=0.04`, `relax_steps=50`
- Training: 120 epochs, batch size 48
- Result: Both PC and MLP reached ~100% accuracy on this easy dataset

### Phase 2 — Scaling difficulty (current)

Restructured script to run four configs in sequence to find where PC starts to lag behind Adam/backprop.

- Network shape upgraded to `2 → 64 → 32 → N_classes`
- PC hyperparams: `α=0.05`, `β=0.02`, `relax_steps=50`
- Training: 150 epochs, batch size 64, 200 samples/class
- Threshold reported: first epoch reaching ≥95% test accuracy

| Config | Classes | Noise | Difficulty |
|---|---|---|---|
| Baseline | 3 | 0.5 | Easy (well-separated) |
| High overlap | 3 | 1.5 | Blobs strongly overlapping |
| More classes | 6 | 0.5 | Larger output space |
| Hard | 6 | 1.5 | Both overlap and more classes |

**Key question:** Can PC's local Hebbian updates reach the same accuracy as Adam+backprop, and how quickly, as the task gets harder?

## PC update rules (no backprop)

```
Prediction:   pred_l = relu(x[l+1]) @ W[l].T
Error:        eps_l  = x[l] - pred_l
Activity:     Δx_l   = α(−eps_l + eps_{l−1} @ W[l−1])
Weight:       ΔW_l  += β * eps_l.T @ relu(x[l+1]) / batch_size
```

Input clamped to data; output clamped to one-hot target during training. Activities relax for `relax_steps` iterations before weights are updated.

## Next steps

- Run the 4-config experiment and record where (if ever) PC falls behind MLP
- Try deeper networks or more relax steps for the harder configs
- Explore weight decay / momentum in the Hebbian update to improve stability
- Consider MNIST as a next real-world benchmark


---

# Visual Active Inference: Camera-Based Robot Arm Control (`pc_visual_robot.py`)

Extends the 1-D active inference in `pc_robot.py` to **2-D camera space**.
A colored marker on the gripper is tracked by a (simulated) camera; the PC brain drives the arm toward a goal by minimising the visual prediction error — **no inverse kinematics, no knowledge of arm geometry**.

## Core idea

Traditional robot control needs the arm's exact geometry to compute joint angles algebraically (inverse kinematics).  Visual Active Inference avoids this entirely:

1. The camera reports the pixel coordinates of a marker on the gripper.
2. A PC state estimator filters those noisy readings into a smooth 2-D belief μ.
3. An active inference controller generates a Cartesian velocity command:
   `u = κ_goal · (goal_xy − μ)`
4. Hardware maps that velocity to joint commands (Jacobian on the MCU).

The PC brain sees only pixels and a goal position.  It never touches joint angles.

## Files

| File | Role |
|---|---|
| `pc_visual_robot.py` | Simulation, PC state estimator, active inference controller, hardware notes |

## Architecture

```
Camera (pixel reading y)
        │
        ▼
┌────────────────────┐
│  PCStateEstimator2D│  filters noise → smooth belief μ = (x̂, ŷ)
│  κ_s · ε_s − κ_p · ε_p │
└────────────────────┘
        │  μ
        ▼
┌───────────────────────────┐
│  ActiveInferenceController│  u = κ_goal · (goal − μ)
│  (only sees pixel coords) │
└───────────────────────────┘
        │  v_xy (Cartesian velocity)
        ▼
┌──────────────────┐
│  Jacobian mapper │  dθ = J⁺ · v_xy     ← runs on motor controller / MCU
│  (arm geometry)  │                        PC brain never uses this
└──────────────────┘
        │
        ▼
     Joint motors
...
prior error   : ε_p = μ − μ₀
belief update : Δμ  = α (κ_s · ε_s − κ_p · ε_p)
```

- `κ_s` — how much to trust the camera (high = follow camera closely)
- `κ_p` — how much to penalise sudden jumps (high = smooth/damped motion)

## Running the simulation

```bash
python3 pc_visual_robot.py
```

Output:
- Step-by-step table: true arm position, PC belief, P-controller position, distance to goal
- ASCII 2-D map of the marker trajectories (PC vs plain proportional controller)
- Noise-reduction stats comparing raw camera RMSE vs PC belief RMSE
- High-noise second run showing PC filtering advantage
- Hardware connection guide (OpenCV color tracking, Arduino/ROS motor commands, tuning table)

## Connecting to real hardware

**Camera** — replace `arm.camera_reading()` with OpenCV color-blob tracking:
```python
hsv  = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
mask = cv2.inRange(hsv, lower_red, upper_red)
M    = cv2.moments(mask)
y_cam = np.array([M['m10']/M['m00'], M['m01']/M['m00']])
```

**Goal** — take a photo of the arm at the target position; record the marker pixel coords.

**Motors** — replace `arm.apply_cartesian_velocity(v)` with a serial/ROS command:
```python
ser.write(f"{v[0]:.3f},{v[1]:.3f}\n".encode())   # Arduino receives vx, vy
```

**No calibration needed** — the PC loop zeroes out the visual error regardless of arm geometry.  If you push the arm away, the prediction error grows and the controller automatically corrects.

## Tuning guide

| Symptom | Fix |
|---|---|
| Arm too slow / stops short | increase `kappa_goal` (1.0 – 3.0) |
...
| Motor jitter at goal | lower `max_speed` or add a dead-band threshold |

## Comparison vs `pc_robot.py`

| Aspect | `pc_robot.py` | `pc_visual_robot.py` |
|---|---|---|
| State space | 1-D position on a track | 2-D (x, y) in camera/Cartesian space |
| Sensor | Noisy position encoder | Simulated camera (color marker) |
| Controller output | Scalar force | 2-D Cartesian velocity vector |
| Geometry needed? | No (1-D, trivial) | No — Jacobian only on the MCU |
| Baseline | PID | Proportional (raw camera) |

## Next steps

- Replace simulated camera with real OpenCV color tracking
- Extend to a 3-DOF arm (add a wrist joint) — state estimator is unchanged
- Add a second visual goal (path following) using `controller.set_goal()`
- Use the PC classifier from `pc_torch.py` as the generative model A so the robot learns its own observation model from experience
